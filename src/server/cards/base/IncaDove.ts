import { BirdCard } from '../BirdCard';
import { BirdCardName } from '../../../common/cards/BirdCardName';
import { FoodType } from '../../../common/game/FoodType';
import { NestType } from '../../../common/game/NestType';
import { HabitatType } from '../../../common/game/HabitatType';
import { PowerType } from '../../../common/game/PowerType';
import type { Player } from '../../Player';
import type { Game } from '../../Game';
import { LayEggs } from '../../deferredActions/LayEggs';

export class IncaDove extends BirdCard {
  readonly name = BirdCardName.INCA_DOVE;
  readonly commonName = 'Inca Dove';
  readonly scientificName = 'Columbina inca';
  readonly habitats = [HabitatType.GRASSLAND];
  readonly foodCost = [FoodType.SEED, FoodType.SEED];
  readonly nestType = NestType.PLATFORM;
  readonly eggCapacity = 4;
  readonly wingspan = 28;
  readonly points = 2;
  readonly powerType = PowerType.WHITE;
  readonly powerText = 'Lay 1 egg on each of your birds with a platform nest.';

  onPlay(player: Player, _game: Game): void {
    for (const bird of player.board.getAllBirds()) {
      const birdCard = _game.createBirdCardInstance(bird.name as any);
      if (birdCard && birdCard.nestType === NestType.PLATFORM && bird.eggs < birdCard.eggCapacity) {
        bird.eggs++;
      }
    }
  }
}
