...
/** Points for birds that eat invertebrates. */
export class FoodWebExpert extends BonusCard {
  ...
  readonly description = '1 point for each bird that has invertebrate in its food cost.';

  score(player: Player): number {
    let count = 0;
    for (const bird of player.board.getAllBirds()) {
      const card = createBirdCard(bird.name as BirdCardName);
      if (card && card.foodCost.includes(FoodType.INVERTEBRATE)) {
        count++;
      }
    }
    return count;
